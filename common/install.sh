...
if $PATCH; then
  ...
  for FLAG in "deep_buffer" "raw" "low_latency"; do
    if [ -f $VEN/etc/audio_output_policy.conf ] && [ -f /system/etc/audio_policy_configuration.xml ]; then
      $FIRST && cp_ch $ORIGDIR/system/etc/audio_policy_configuration.xml $UNITY/system/etc/audio_policy_configuration.xml
      ...
      $FIRST && { cp_ch $ORIGDIR$VEN/etc/audio/audio_policy_configuration.xml $UNITY$VEN/etc/audio/audio_policy_configuration.xml;
      ...
      for OFILE in ${POLS}; do
        FILE="$UNITY$(echo $OFILE | sed "s|^/vendor|/system/vendor|g")"
        $FIRST && cp_ch $ORIGDIR$OFILE $FILE
        case $FILE in
