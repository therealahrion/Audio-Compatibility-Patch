if ! $MAGISK || $SYSOVERRIDE; then
  if $(grep_prop patch $MOD_VER); then
    for OFILE in ${POLS}; do
      ...
      case $FILE in
        *.xml) sed -ri "/<mixPort name=\"(deep_buffer)|(raw)|(low_latency)|(primary-out)\"/,/<\/mixPort>/ {/<!--/!{/flags=\"AUDIO_OUTPUT_FLAG_.*\"/d}; s|( *)<!--(.*flags=\".*\".*)$MODID-->|\1\2|}" $FILE;;
        *.conf) sed -ri "/^ *(deep_buffer)|(raw)|(low_latency)|(primary) \{/,/}/ {/^ *flags AUDIO_OUTPUT_FLAG_.*$/d; s|#$MODID||}" $FILE;;
      esac
    done
  else
